...
+(SSChatMessagelLayout *)getMessageWithDic:(NSDictionary *)dic{
    ...
    if(messageFrom == SSChatMessageFromMe){
        ...
        message.backImgString = @"orange";
    }else{
        message.messageFrom = SSChatMessageFromOther;
        message.backImgString = @"white";
    }
...
//    }
    ...
    if(message.messageType == SSChatMessageTypeText){
        ...
        if([dic[@"image"] isKindOfClass:[UIImage class]]){
            ...
            message.image = [UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:dic[@"image"]]]];
        }
...
+(void)sendMessage:(NSDictionary *)dict sessionId:(NSString *)sessionId messageType:(SSChatMessageType)messageType messageBlock:(MessageBlock)messageBlock{
    ...
    switch (messageType) {
        case SSChatMessageTypeText:{
            ...
            [[V2TIMManager sharedInstance] sendC2CTextMessage:dict[@"text"] to:sessionId succ:^{
                XYLog(@"chenggong");
            } fail:^(int code, NSString *desc) {
                XYLog(@"fail>>>%d,%@",code,desc);
            }];
        }
            break;
        case SSChatMessageTypeImage:{
            ...
            [messageDic setValue:sessionId forKey:@"sessionId"];
            [messageDic setValue:headerImg1 forKey:@"headerImg"];
            NSData *data = UIImageJPEGRepresentation((UIImage*)dict[@"image"], 0.75);
            NSString *path = [TUIKit_Image_Path stringByAppendingString:[THelper genImageName:nil]];
            [[NSFileManager defaultManager] createFileAtPath:path contents:data attributes:nil];
            XYLog(@"path>>%@",path);
            V2TIMMessage *msg = [[V2TIMManager sharedInstance] createImageMessage:path];
            [[V2TIMManager sharedInstance] sendMessage:msg receiver:sessionId groupID:@"" priority:V2TIM_PRIORITY_DEFAULT onlineUserOnly:NO offlinePushInfo:nil progress:^(uint32_t progress) {
