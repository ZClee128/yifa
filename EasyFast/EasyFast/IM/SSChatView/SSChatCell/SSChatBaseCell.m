...
-(void)setLayout:(SSChatMessagelLayout *)layout{
    ...
    [self.mHeaderImgBtn setImage:[UIImage imageNamed:@"header"] forState:UIControlStateNormal];
//    [self.mHeaderImgBtn.imageView sd_setImageWithURL:[NSURL URLWithString:layout.message.headerImgurl]];
    self.mHeaderImgBtn.layer.cornerRadius = self.mHeaderImgBtn.height*0.5;
    if(_layout.message.messageFrom == SSChatMessageFromOther){
        [self.mHeaderImgBtn setImage:[UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:layout.message.headerImgurl]]] forState:(UIControlStateNormal)];
    }
