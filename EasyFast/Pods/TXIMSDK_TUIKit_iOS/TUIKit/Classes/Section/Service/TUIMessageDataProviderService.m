...
- (NSString *)getCustomElemContent:(V2TIMMessage *)message {
    ...
    if (param != nil) {
        ...
        NSString *businiessID = param[@"businessID"];
        if (version == Version) {
            if ([businiessID isEqualToString:@"group_create"]) {
                return [NSString stringWithFormat:@"\"%@\"%@",param[@"opUser"],param[@"content"]];
            }
            // 音视频通话邀请 APNs 消息，不展示
            if ([businiessID isEqualToString:@"av_call"]) {
                return @"";
            }
        }
        // 兼容下 v1 版本的群创建
        if ([param.allKeys containsObject:@"group_create"]) {
